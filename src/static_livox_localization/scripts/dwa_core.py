...
class DwaPlanner:
    ...
    def plan(self, state, obstacles=(), speed_cap=None, last_yaw_rate=0.0,
        ...
        # A downstream safety gate may know geometry that the clustered
        # obstacle critic cannot: notably the raw LiDAR swept footprint.
        # Ask it in score order rather than accepting the cheapest candidate
        # and discovering one node later that the command can never pass.
        # The callback returns True to reject a candidate.  It is deliberately
        # optional so ordinary route following retains the vectorized hot
        # path; stationary-person bypass is the only current caller.

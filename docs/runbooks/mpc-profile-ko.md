...
## 5.1 측정 주행 전에 꺼야 할 것 (2026-08-05 실측)

**RViz와 원격 데스크톱을 끄고 재십시오.** MPC 주행 중 제어 루프가 이렇게 돌았습니다:

| | 값 |
|---|---|
| 중앙값 | 0.194 s (**5.2 Hz**, 목표 10 Hz) |
| p90 | 0.813 s |
| p99 | 2.137 s |
| 최대 | 3.763 s |
| 0.1 s 초과 | 72% |

그때 CPU 상위는 `fastlio_mapping 47%`, `Xorg 26%`, `rustdesk 15%`, `livox 6%`, `rviz 4%`
였습니다. **Xorg와 rustdesk와 RViz를 합치면 45%** 인데 셋 다 관측 도구지 스택이 아닙니다.
i7-1165G7은 4코어 28 W 지속이라 여기에 주행 체인과 블랙박스가 더 붙으면 스로틀링까지 겹칩니다.

```bash
RVIZ=false PROFILE=mpc ~/start_wheelchair_localization.sh
```

원격으로 보셔야 하면 rustdesk 대신 `VNC=1`을 쓰고, 화면을 보는 동안에는 측정값을
신뢰하지 마십시오. 예산 게이트(40 ms)와 2차 SQP 게이트(16 ms)는 **벽시계 기준**이라
CPU 작업량이 아니라 스케줄링 지연으로도 터집니다. 그래서 `REUSED`가 뜨면 그것이
솔버가 느린 것인지 기계가 바쁜 것인지 구분되지 않습니다 — 화면을 끄고 다시 재는 것이
그 구분의 유일한 방법입니다.
...
| 실차 조향 감각 | 미확인 |
| 제어 루프가 10 Hz를 못 지킴 | 2026-08-05 실측 중앙 5.2 Hz. 명령 램프는 실경과시간을 쓰도록 고쳤으나(`mpc_command.py`) 루프 자체는 여전히 부하에 밀림. RViz·rustdesk를 끄고 재측정 필요 |

...
ROUTE="${ROUTE:-$REPO/routes/20260814_route_algorithm_waypoints.json}"
BAND="${BAND:-$REPO/routes/20260814_route_algorithm_safety_band.json}"
DRIVABLE_MASK="${DRIVABLE_MASK:-$REPO/routes/route_2d_map_algorithm.yaml}"
...
cleanup() {
  ...
  if [ -n "$OWN_ROSCORE_PID" ]; then
    kill -- "-$OWN_ROSCORE_PID" 2>/dev/null ||
      kill "$OWN_ROSCORE_PID" 2>/dev/null || true
    wait "$OWN_ROSCORE_PID" 2>/dev/null || true
  fi
  {
